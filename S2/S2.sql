...
-- [OP050] Comprobamos que los datos se cargaron
...
-- [OP052] Comprobamos que los datos se cargaron
...
-- [OP054] Comprobamos que los datos se cargaron
...
-- [OP060] Observamos antes de la operacion
SELECT *
FROM transactions AS t
WHERE t.id = '000447FE-B650-4DCF-85DE-C7ED0EE1CAAD';

-- [OP061] Eliminamos la transaccion
DELETE FROM transactions AS t
WHERE t.id = '000447FE-B650-4DCF-85DE-C7ED0EE1CAAD';

-- [OP062] Observamos que ha sido eliminada
SELECT *
FROM transactions AS t
WHERE t.id = '000447FE-B650-4DCF-85DE-C7ED0EE1CAAD';
...
-- [OP067] Observamos la tala
SELECT * FROM cards_activity;

-- [OP068] Cuantas tarjetas son activas?
...
-- A diferenecia de mi version anterior, esta solucion es
-- simple: primero cogemos "product_ids" y le añadimos
-- corchetes para que la lista sea un JSON array y se
-- considere JSON valido. Despues usando JSON query
-- seleccionamos todos los elementos en el array, 
-- cada uno lo converimos en una fila, leyendo el string
-- como una lista de INT que JSON_TABLE se encarga de
-- transformar en el formato binario correcto. El JOIN
-- con JSON_TABLE dependiente en elementos se t se trata
-- como un JOIN LATERAL por defecto.
-- [OP071]
INSERT INTO orders
(transaction_id, product_id)
SELECT
